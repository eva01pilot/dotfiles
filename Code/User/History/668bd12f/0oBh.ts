...
export default async function handler(req : NextApiRequest, res: NextApiResponse){
    if(req.method==='POST'){
        ...
        const hashedPassword = await generatePasswordHash(10, req.body.password as string)
        ...
        const accessToken = jwt.sign({login:User.id}, process.env.JWT_SECRET as string,{
            expiresIn: 36000
        })
