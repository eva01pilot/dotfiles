...
const save = async () => {
    ...
    if (res.result === true) {
        ...
        const { ENTITY_ID, ENTITY_VALUE_ID } = placement.options
        const entity = String(ENTITY_ID).split('_')[1].toLowerCase()
        const res = await bitrix.call(`crm.${entity}.get`, {
            id: ENTITY_VALUE_ID
        })
        const ent = res.result
        blurText.value = `${ent.LAST_NAME ?? ''} ${ent.NAME ?? ''} ${ent.SECOND_NAME ?? ''}`.replace(/\s+/g, ' ').trim()
...
<template>
    <div v-if="inputFocused" class="flex flex-col">
        <div class="p-2">
            <a-auto-complete ref="dropdown" v-model:value="value" :options="options" class="w-full z-50 isolate"
                @select="onSelect" @search="onSearch" @focus="() => inputFocused = true" @dropdownVisibleChange="onChangeVis"
