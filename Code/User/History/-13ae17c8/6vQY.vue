...
<template>
    <li :class="`flex flex-row justify-between  ${!last && 'border-b border-b-gray-200'} p-2`">
        ...
        <div class="flex flex-row items-center max-w-[3.3rem]">
            <span class="font-bold text-[.8rem]  p-1 text-ellipsis whitespace-nowrap overflow-hidden">{{ value }}</span>
            <span class="text-gray-900 text-[.8rem]" v-if="suffix">{{ suffix }}</span>
        </div>
    </li>
</template>