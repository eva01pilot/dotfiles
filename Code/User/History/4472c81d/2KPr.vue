...
const startParsing = async () => {
    ...
    for(const entity  of entitiesTypes){
        ...
        while (nstart || nstart === 0) {
            ...
            nstart = res.next
            entities.get(entity as 'lead'|'contact')?.push(res.result.slice(0, countValues.value[entity as 'lead' | 'contact']))
            console.log(countValues.value[entity as 'lead'|'contact'])
            if(nstart>=countValues.value[entity as 'lead'|'contact']) {
                entities.set(entity as 'lead' | 'contact', entities.get(entity as 'lead' | 'contact')?.slice(0, countValues.value[entity as 'lead'|'contact']) as any[]) 
                break
            }
