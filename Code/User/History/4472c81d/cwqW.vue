        ...
        <div class="h-5/6 flex items-center justify-between flex-col md:flex-row">
            ...
            <div class="grid grid-cols-2 gap-2">
                <a-card v-if="ready['contact']">
                    <a-statistic decimalSeparator="" title="Исправлено контактов" :value="ready['contact'].current" :suffix="`/${ready['contact'].total}`"  />
                </a-card>
                <a-card v-if="ready['lead']">
                    <a-statistic decimalSeparator="" title="Исправлено лидов" :value="ready['lead'].current" :suffix="`/${ready['lead'].total}`"  />
                </a-card>
            </div>
        </div>
        <a-button :disabled="disabled" @click="startParsing"
