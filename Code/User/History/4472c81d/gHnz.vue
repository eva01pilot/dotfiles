...
const disabled = computed(() =>
    (!checked.value['contact'] && !checked.value['lead']) || !(usedApi.value === 'ahunter')
)
...
const startParsing = async () => {
    ...
    for(const entity  of entitiesTypes){
        ...
        while (nstart || nstart === 0) {
            ...
            nstart = res.next
            ready.value[entity as 'lead'|'contact'].total = res.total
        ...
        if (!entities.get(entity as 'lead'|'contact')) return
        ...
        ready.value[entity as 'lead'|'contact'].current += promises.length
    }
    loading.value = false
...
const onElementVisibility = async(state: boolean) =>{
    console.log(state);
    
    const usedRes = await bitrix.app.option.get('usedApi')
    if(state) usedApi.value = usedRes.result
}
</script>

<template>
    <div v-element-visibility="onElementVisibility" class="w-full h-[calc(100vh-10vh)] max-h-[calc(100vh-10vh)] overflow-y-scroll md:overflow-y-hidden flex justify-between items-center flex-col">
        <div class="h-5/6 flex items-center justify-between flex-col md:flex-row">
                    ...
                    <div class="grid grid-cols-[1fr_1fr] gap-2 py-2">
                        ...
                        <div>
                            <a-tooltip>
                                ...
                                Ограничение сверху:
                                <a-input-number v-model:value="countValues['lead']" style="width: 100%;"
                    ...
                    <div class="grid grid-cols-[1fr_1fr] gap-2 py-2">
                        ...
                        <div>
                            <a-tooltip>
                                ...
                                Ограничение сверху:
                                <a-input-number v-model:value="countValues['contact']" style="width: 100%;"
            ...
            <div class="grid grid-cols-2 gap-2">
                <a-card v-if="ready['contact']">
                    <a-statistic v-if="ready['contact'].total" title="Обновлено контактов" :value="ready['contact'].current" :suffix="`/${ready['contact'].total}`"  />
                    <a-statistic v-if="ready['contact'].total" title="Валидных контактов" :value="ready['contact'].total - ready['contact'].current" :suffix="`/${ready['contact'].total}`"  />
                </a-card>
                <a-card v-if="ready['lead']">
                    <a-statistic v-if="ready['lead'].total" title="Обновлено лидов" :value="ready['lead'].current" :suffix="`/${ready['lead'].total}`"  />
                    <a-statistic v-if="ready['lead'].total" title="Валидных лидов" :value="ready['lead'].total - ready['lead'].current" :suffix="`/${ready['lead'].total}`"  />
                </a-card>
            </div>
        </div>
        <a-button :loading="loading" v-if="usedApi === 'ahunter'" :disabled="disabled" @click="startParsing"
            class="my-2">Запустить валидацию</a-button>
        
        <p v-else>Валидация работает только с API ahunter. API ключ для этого не требуется</p>
