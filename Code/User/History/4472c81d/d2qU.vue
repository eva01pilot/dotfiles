...
const disabled = computed(() =>
    (!checked.value['contact'] && !checked.value['lead']) || !(props.api === 'ahunter')
)
...
const startParsing = async () => {
    ready.value.contact = {
        current: 0,
        total: countValues.value['contact']
    }
    ready.value.lead = {
        current: 0,
        total: countValues.value['lead']
    }
    ...
    for(const entity  of entitiesTypes){
        ...
        ready.value[entity as 'lead'|'contact'].current = countValues.value[entity as 'lead' | 'contact']
        ...
        <div class="h-5/6 flex items-center justify-between flex-col md:flex-row">
            ...
            <div class="grid grid-cols-2 gap-2">
                <a-card v-if="ready['contact']">
                    <a-statistic v-if="ready['contact'].total" title="Исправлено контактов" :value="ready['contact'].current" :suffix="`/${ready['contact'].total}`"  />
                    <a-statistic v-if="ready['contact'].total" title="Всего нужно исправить" :value="countValues['contact'] - ready['contact'].current" :suffix="`/${countValues['contact']}`"  />
                </a-card>
                <a-card v-if="ready['lead']">
                    <a-statistic v-if="ready['lead'].total" title="Исправлено лидов" :value="ready['lead'].current" :suffix="`/${ready['lead'].total}`"  />
                    <a-statistic v-if="ready['lead'].total" title="Всего нужно исправить" :value="countValues['lead'] - ready['lead'].current" :suffix="`/${countValues['lead']}`"  />
                </a-card>
            </div>
        </div>
        <a-button :loading="loading" v-if="api === 'ahunter'" :disabled="disabled" @click="startParsing"
