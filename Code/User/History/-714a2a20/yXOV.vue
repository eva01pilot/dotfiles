...
const usedApi = ref<'ahunter'|'dadata'>('ahunter')
...
const switchApi = async (api: API, event: boolean) => {
    ...
    await bitrix.app.option.set({
        'usedApi': Object.keys(checked.value).filter((key) => checked.value[key as API])[0],
    })
    usedApi.value = Object.keys(checked.value).filter((key) => checked.value[key as API])[0] as API
}
...
const addFields = async () => {
    await bitrix.call('userfieldtype.add', {
        ...
        TITLE: 'Автозаполнение ФИО',
        DESCRIPTION: 'Поле с подсказками по ФИО'
    })
    await bitrix.call('crm.lead.userfield.add', {
        fields: {
            "FIELD_NAME": "NEBO_FIO",
            "EDIT_FORM_LABEL": "Автозаполнение ФИО",
            "LIST_COLUMN_LABEL": "Автозаполнение ФИО",
            "USER_TYPE_ID": "nebo_fio",
            "XML_ID": "NEBO_FIO",
        }
    })
...
<template>
    <div class="h-screen flex flex-col justify-center items-center overflow-y-scroll md:overflow-y-hidden">
        <a-tabs  v-if="isAdmin">
            <a-tab-pane key="1" tab="Валидация ФИО">
                <Parse :api="usedApi as 'ahunter'|'dadata'"/>
            </a-tab-pane>
            <a-tab-pane key="2" tab="Настройки приложения">
        ...
        <h1 v-else>Вы не можете редактировать настройки приложения, поскольку вы не являетесь админом</h1>
    </div>
</template>
<style scoped>
.divider {
