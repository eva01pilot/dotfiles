...
onBeforeMount(async () => {
    ...
    const domain = window.name ? `https://${await getDomain()}` : window.location.origin
    const member_id = tokenStore.memberID || (await getAuth() as any)['member_id']
...
const onMapCreated = (y: ymaps.Map) => {
    ...
    mapinstance.value.controls.remove('fullscreenControl')
}
...
const onTimeChange = async (date: any, datestring: string) => {
    ...
    pathsAndDots.clear()
    const domain = window.name ? await `https://${getDomain()}` : window.location.origin
    const member_id = tokenStore.memberID || (await getAuth() as any)['member_id']
        ...
        if (user.checked) {
            ...
            placemarkArray.forEach((point, index) => {
                ...
                point.events.add(enterType, () => {
                    ...
                    window.innerWidth < 768 && mapinstance.value.events.add('click', ()=>{
                        markArray.forEach((mark)=>{
                            mapinstance.value?.geoObjects.remove(mark)
                    })
                    })
