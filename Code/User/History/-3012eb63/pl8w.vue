...
import { getAuth } from '@sknebo/bitrix-js';
...
const saveClipboard = async (ID: string) => {
    const text = `t.me/catsimages_bot?start=${store.memberID || (await getAuth() as any)['member_id']}_${ID}`
...
onMounted(async () => {
    let start = 0
    loading.value = true
    const total = (await bitrix.call('user.search')).total
    let count50 = 0
    const startArr = []
    while(count50<total){
        startArr.push(count50)
        count50 += 50
    }
    const promisesList = startArr.map((start)=>({
        [start]: {
            method: "user.search",
            params: {
                start,
    ...
    for (let i = 0; i < promisesList.length; i++) {
        batchList[Object.keys(promisesList[i])[0]] = Object.values(promisesList[i])[0];
    }
    const res = await bitrix.batch(batchList)
    const usersResult = res.result.result[0]
...
<template>
    <a-list :locale="{  
        ...
        }" :loading="loading" :data-source="users" item-layout="horizontal" :pagination="calcPagination()">
        <template #header>
            <div class="p-2">
