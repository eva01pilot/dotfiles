...
<div class="max-w-[20.5rem] sm:max-w-screen-sm md:max-w-[100%] p-2 overflow-auto">
    <table class=" md:w-full  p-4 block md:table  overflow-auto whitespace-nowrap">
        ...
        <tbody class="w-full">
            <tr class="rounded-lg" v-for="item in transactions">
                <td class="text-left p-3 text-sm rounded-lg">{{ item.title }} <b>{{ item.name }}</b></td>
                <td class="text-left p-3 text-sm rounded-lg">{{ item.date }}</td>
