...
import { ref } from 'vue';
import bitrixLogin from '../bitrix_login';
...
const onLogin = async(e:Event) =>{
    e.preventDefault()
    const res = await bitrixLogin.call('client.data.auth.authenticate',{
        login: login.value,
        password: password.value
    })
    if(res.result.tokens){
        store.state.tokens = res.result
        const {access_token, refresh_token, expires_in} = store.state.tokens
        bitrix.oauth?.setCredentials(access_token, refresh_token, expires_in)
        router.push('/')
    }
