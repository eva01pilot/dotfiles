...
export default async function handler(req:NextApiRequest, res:NextApiResponse){
    if(req.method==='GET'){
        ...
        const verification = jwt.verify(access_token as string, process.env.JWT_SECRET as string)
        console.log(verification)
        
        if(!access_token || !jwt.verify(access_token as string, process.env.JWT_SECRET as string)){
            return res.json({
                login:false
            })
        }
